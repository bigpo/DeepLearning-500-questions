...
AP是指Average Precision，是在不同召回率下的精确度的平均。
...
Mask R-CNN相较于Faster R-CNN主要是多了一个预测mask的分支，而且改进了ROI Pooling层的操作，称为RoI Align。

**1. RoI Align 与 RoI Pooling的区别**

先说下RoI Pooling，作用是将proposal对应的特征图区域池化为固定尺寸大小，以便于接入全连接层。在操作中存在两次量化误差：

+ 将proposal的尺寸除以stride，得到对应于feature map上的区域，这个可能会产生浮点数
+ 将上一步得到的区域再分割为 7 * 7，又会产生浮点数

RoI Pooling操作对于上述的步骤是直接向下取整，从而使最终得到的特征区域存在偏差。

RoI Align是如何解决这个问题的？RoI Align没有对特征区域的坐标直接量化取整，而是采用双线性插值的方法求取采样点位置处的特征值，依据采样点数量的不同，有1个或4个的，论文称采取4个采样点的效果最好。

![roi_align1](/Users/songpo/Documents/DeepLearning-500-questions/ch08_目标检测/img/ch8/roi_align1.png)

**2. RoI Pooling和RoI Align的反向传播？**
...
作者对one-stage检测器准确率不高的问题进行探究，发现主要问题在于**正负类别不均衡**（简单-难分类别不均衡）。
